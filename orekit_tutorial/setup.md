...
**在安装任何 Python 包时，请优先使用 `conda` 命令，而不是 `pip`。**

| 命令 | 推荐度 | 说明 |
|------|--------|------|
| `conda install xxx` | ⭐⭐⭐ 推荐 | 自动处理依赖，版本兼容性好 |
| `pip install xxx` | ⚠️ 备选 | 可能导致依赖冲突 |

**原因**：
- Conda 会自动检查包之间的兼容性
- Orekit 需要 Java 环境，Conda 能正确安装 openjdk
- 混用 conda 和 pip 可能导致环境损坏
...
**重要：使用 conda 安装，不要使用 pip**

```bash
# 安装 numpy（数值计算）- 使用 conda
conda install numpy

# 安装 matplotlib（绘图）- 使用 conda
...
# 1. 首先检查 JPype 是否安装
conda activate orekit_env
python -c "import jpype; print(f'JPype {jpype.__version__}')"

# 如果报错 ModuleNotFoundError: No module named 'jpype'
# 说明 JPype 没有安装，执行：
pip install jpype1
# 或
conda install -c conda-forge jpype1

# 2. 如果 JPype 已安装但仍报错，需要找到你的 jvm.dll 路径
#    首先查找 conda 环境的安装位置：
where conda

#    通常路径为：C:\Users\你的用户名\anaconda3\envs\orekit_env\
#    或者：C:\Users\你的用户名\miniconda3\envs\orekit_env\
#    
#    然后检查 jvm.dll 是否存在：
dir "C:\Users\你的用户名\anaconda3\envs\orekit_env\Library\jre\bin\server\jvm.dll"
#    
#    将"你的用户名"替换为你的 Windows 用户名

# 3. 如果文件存在但仍报错，重新安装 openjdk
conda install -c conda-forge openjdk=17 --force-reinstall
```

### Q4: `ImportError: cannot import name 'PositionAngle' from 'org.orekit.orbits'`

**原因**：Orekit 版本不匹配（安装了 13.x 但教程需要 11.3.3）
...
# 2. 如果版本不是 11.3.3，需要降级
conda remove orekit -c conda-forge
conda install -c conda-forge orekit=11.3.3

# 3. 验证版本
python -c "import orekit; print(f'Orekit {orekit.VERSION}')"

# 4. 测试 PositionAngle 导入
python -c "
import orekit; orekit.initVM()
from org.orekit.orbits import KeplerianOrbit, OrbitType, PositionAngle
print('PositionAngle 导入成功')
"
```

### Q5: Orekit 版本不是 11.3.3
